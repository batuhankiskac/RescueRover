...
namespace {
	...
	uint16_t		medianDistance() {
		uint16_t		sorted[3] = {distanceSamples[0], distanceSamples[1],
		distanceSamples[2]};
	...
	void			readDistance() {
		...
		pulseIn(Pins::ULTRASONIC_ECHO, HIGH, ULTRASONIC_TIMEOUT_US);
		const			uint16_t centimeters = static_cast < uint16_t > (durationUs / 58UL);

		if				(durationUs == 0 || centimeters < ULTRASONIC_MIN_CM ||
						 centimeters > ULTRASONIC_MAX_CM) {
	...
	void			readGas() {
		...
		if (!gasFilterInitialized) {
			...
			static_cast < int16_t > (data.gasRaw) -
			static_cast < int16_t > (data.gasFiltered);
							data.gasFiltered = static_cast < uint16_t > (
								static_cast < int16_t > (data.gasFiltered) +
				 difference / static_cast < int16_t > (GAS_FILTER_DIVISOR));
		}
		updateGasState();
	}
	...
	bool			readDht11() {
		...
		waitWhileLevel(HIGH, 120) != 0 && waitWhileLevel(LOW, 120) != 0 &&
		waitWhileLevel(HIGH, 120) != 0;
		if				(!responseOk) {
			data.dhtValid = false;
			return false;
		}
		...
		if				(checksum != bytes[4]) {
			...
			return false;
						...
						data.humidityDeciPct =
		static_cast < uint16_t > (bytes[0]) * 10U + bytes[1];
		int16_t			temperature =
		static_cast < int16_t > (bytes[2] & 0x7FU) * 10 + bytes[3];
		if				((bytes[2] & 0x80U) != 0U) {
	...
	bool			mpuPresent() {
		...
		if (Wire.requestFrom(static_cast < uint8_t > (MPU6050_ADDRESS),
	...
	int16_t			readI16() {
		...
		const			uint8_t lowByte = Wire.read();
						return static_cast < int16_t > ((static_cast < uint16_t > (highByte) << 8U) |
														lowByte);
	}
	...
	void			readMpu() {
		...
		Wire.write(0x3B);
		if (Wire.endTransmission(false) != 0 ||
			Wire.requestFrom(static_cast < uint8_t > (MPU6050_ADDRESS),
		...
		const float		roll = atan2f(static_cast < float >(ay), static_cast < float >(az))*RAD_TO_DEG_F +
		MPU_ROLL_OFFSET_DEG;
		const float		pitch =
		atan2f(-static_cast < float >(ax),
			   sqrtf(static_cast < float >(ay) * static_cast < float >(ay) +
					 static_cast < float >(az) * static_cast < float >(az)))*RAD_TO_DEG_F +
		MPU_PITCH_OFFSET_DEG;
		...
		if				(!mpuFilterInitialized) {
			...
			data.rollDeg = static_cast < int16_t > (
				(static_cast < int32_t > (data.rollDeg) * 3 + newRoll) / 4);
			data.pitchDeg = static_cast < int16_t > (
			  (static_cast < int32_t > (data.pitchDeg) * 3 + newPitch) / 4);
		}
						data.mpuValid = true;
	}
	...
	void			readPir(bool motorsMoving){
		...
		const bool		current = data.pirReady && !motorsMoving &&
		digitalRead(Pins::PIR) == HIGH;
	...
	void			updateSoundWindow(uint32_t nowMs) {
		...
		while			(static_cast < int32_t > (nowUs - nextSoundSampleUs) >= 0 &&
						 samplesThisPass < 4U) {
			const			uint16_t sample =
			static_cast < uint16_t > (analogRead(Pins::SOUND_ANALOG));
		...
		if				(!elapsed(nowMs, soundWindowStartMs, SOUND_SAMPLE_WINDOW_MS)) {
			return;
						...
						data.soundAmplitude =
		soundSampleCount > 1U ? soundMaximum - soundMinimum : 0U;
...
digitalWrite(Pins::ULTRASONIC_TRIG, LOW);
	...
	const bool		configured = writeMpuRegister(0x6B, 0x00) &&
...
void
sensorsUpdate(uint32_t nowMs, bool motorsMoving, bool scanMode)
{
	...
	if (!motorsMoving && !scanMode && !soundWindowRunning &&
		elapsed(nowMs, lastSoundWindowMs, SOUND_IDLE_PERIOD_MS)) {
...
bool
sensorsStartSoundWindow(uint32_t nowMs)
{
	if (soundWindowRunning) {
		return false;
	}
...
bool
sensorsSoundWindowActive()
{
	return soundWindowRunning;
}

void
sensorsRefreshForScan(uint32_t nowMs)
{
	readPir(false);
	if (elapsed(nowMs, lastDhtMs, DHT_PERIOD_MS)) {
		lastDhtMs = nowMs;
