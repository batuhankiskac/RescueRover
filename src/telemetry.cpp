...
namespace {
	...
	void			printPayload(Print & output, const SensorData & data, RobotState state,
								 bool scan){
		if (scan) {
			output.print(F("SCAN:"));
		}
...
void
telemetrySend(const SensorData & data, RobotState state)
{
...
void
telemetrySendScan(const SensorData & data, RobotState state)
{
...
void
telemetrySendAlert(AlertCode code)
{
