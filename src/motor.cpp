...
void writeSide(uint8_t pin1, uint8_t pin2, int8_t direction, bool reversed) {
    if (reversed) {
        direction = -direction;
    }
...
bool applyMotion(MotionCommand command, int8_t leftDirection, int8_t rightDirection, uint8_t speed) {
    if (allMotionInhibited || (forwardInhibited && command == MotionCommand::FORWARD)) {
        stopMotors();
        return false;
    }

    analogWrite(Pins::MOTOR_ENABLE_PWM, 0);
    writeSide(Pins::MOTOR_LEFT_IN1, Pins::MOTOR_LEFT_IN2, leftDirection, MOTOR_LEFT_REVERSED != 0);
    writeSide(Pins::MOTOR_RIGHT_IN1, Pins::MOTOR_RIGHT_IN2, rightDirection, MOTOR_RIGHT_REVERSED != 0);
...
void motorSetSafetyInhibit(bool blockAllMotion, bool blockForward) {
    ...
    if (allMotionInhibited || (forwardInhibited && status.motion == MotionCommand::FORWARD)) {
        stopMotors();
    }
}
