...
namespace {
    ...
    bool executeMotion(MotionCommand command, uint32_t nowMs) {
        if (!safetyCanMove(command)) {
            telemetrySendAlert(command == MotionCommand::FORWARD
                               ? AlertCode ::FORWARD_BLOCKED
                               :AlertCode::MOTION_BLOCKED);
            outputsPulseAlarm(nowMs, ALERT_PULSE_MS);
            return false;
        }
        ...
        bool accepted = false;
        switch (command) {
            case MotionCommand::FORWARD:accepted = moveForward(selectedSpeed);
            break;
            case MotionCommand::BACKWARD:accepted = moveBackward(selectedSpeed);
            break;
            case MotionCommand::LEFT:accepted = turnLeft(selectedSpeed);
            break;
            case MotionCommand::RIGHT:accepted = turnRight(selectedSpeed);
            break;
    ...
    bool isSpeedCommand(BluetoothCommand command) {
        return command >= BluetoothCommand::SPEED_0 &&
        command <= BluetoothCommand::SPEED_9;
    }
    ...
    void processBluetooth(uint32_t nowMs) {
        ...
        if (isSpeedCommand(command)) {
            const uint8_t digit = static_cast < uint8_t > (command) -
            static_cast < uint8_t > (BluetoothCommand::SPEED_0);
                            selectedSpeed = static_cast < uint8_t > (
                                                           MIN_MOTOR_SPEED +
                                                                     (static_cast < uint16_t > (255U - MIN_MOTOR_SPEED) * digit) / 9U);
                            return;
        }
        ...
        switch (command) {
            ...
            break;
            case BluetoothCommand::CLEAR_EMERGENCY: if(safetyClearEmergency()) {
    ...
    void updateScan(uint32_t nowMs) {
        if (scanPhase == ScanPhase::SETTLING &&
            static_cast < uint32_t > (nowMs - scanPhaseStartMs) >= SCAN_SETTLE_MS) {
            (void)sensorsStartSoundWindow(nowMs);
            scanPhase = ScanPhase::SOUND_WINDOW;
            return;
        }

        if (scanPhase == ScanPhase::SOUND_WINDOW &&
...
void loop() {
    ...
    robotState = calculateRobotState(nowMs);
    const SafetyStatus &safety = safetyGetStatus();
    outputsUpdate(sensorsGetData().dark, safety.warningActive,
                  safety.emergencyActive, nowMs);
